...
def discriminator_loss(logits_real, logits_fake):
    ...
    D_x = bce_loss(logits_real, real_labels)
    D_G_x = bce_loss(logits_fake, fake_labels)
...
def generator_loss(logits_fake):
    ...
    D_G_x = bce_loss(logits_fake, true_labels)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    pre_mean_loss = 0.5 * ((scores_real - 1)**2 + scores_fake**2)
    loss = pre_mean_loss.mean()
...
def ls_generator_loss(scores_fake):
    ...
    pre_mean_loss = 0.5 * (scores_fake - 1)**2
    loss = pre_mean_loss.mean()
