...
def content_loss(content_weight, content_current, content_original):
    ...
    # extract dimensions
    _, C_1, H_1, W_1 = content_current.size()

    # reshape the feaures of the current image to (C X M)
    content_current = content_current.reshape(C_1, H_1 * W_1)

    # reshape the features of the content image to (C X M)
    content_original = content_original.reshape(C_1, H_1 * W_1)

    # content loss given by: 
    #   content weight * sum((current - original)**2)
...
def gram_matrix(features, normalize=True):
    ...
    # flatten dims 2 and 3 in the feature map
    flat_features = features.reshape(N, C, H * W)
    ...
    G = torch.zeros((N, C, C))
    for n in range(N):
      G[n] = torch.matmul(flat_features[n], flat_features[n].T)
...
def style_loss(feats, style_layers, style_targets, style_weights):
    ...
    for i in range(len(style_layers)):
      ...
      a = style_targets[i]
      style_loss += style_weights[i] * torch.sum((g - a)**2)
...
def tv_loss(img, tv_weight):
    ...
    # compute loss (use -1 to let torch figure out the difference bounds)
